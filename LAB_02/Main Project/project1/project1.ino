...
void sendGet(String mac, String key, String value) {
  String url = String(API_BASE) + "/preference?id=" + encodeMac(mac) +
               "&key=" + key + "&value=" + value;
...
void sendPost(String mac, String key, String value) {
  ...
  if (value.length() > 0) {
    mac = value;  // value overrides MAC in id field
  }

  String id = "40256377";
  String finalKey = key.length() > 0 ? key : mac;
  String url = String(API_BASE) + "/preference";
  String body = "id=" + id + "&key=" + finalKey + "&value=" + mac;

  Serial.println("== Simulated cURL ==");
  Serial.println("curl -X POST \"" + url + "\" -H \"Content-Type: application/x-www-form-urlencoded\" -d \"" + body + "\"");
